...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      let isROwner = [global.conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') +91 8592026326'@s.whatsapp.net').includes(m.sender)
      let isOwner = isROwner || m.fromMe
      let isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') +91 8592026326'@s.whatsapp.net').includes(m.sender)
      let isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '') +91 6282670349'@s.whatsapp.net').includes(m.sender)
